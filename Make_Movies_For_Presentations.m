...
[file_xpr,base_path] = uigetfile({'*_xProjDatafile.mat';'*.*'},'Select a file with xProjection data...','G:\Anya\Microscopy_Images\');
...
if strcmp(filename((length(filename)-12):(length(filename)-10)), 'top')
    ...
    x_proj_mov_filename = [base_path '\' filename(1:(length(filename)-13)) 'xProj3.avi'];
else
    if strcmp(filename((length(filename)-15):(length(filename)-10)),'bottom')
        TopBotInd = 1;
        raw_rgb_filename=[base_path '\' filename(1:(length(filename)-16)) 'raw_RGB.tif'];
        x_proj_mov_filename = [base_path '\' filename(1:(length(filename)-16)) 'xProj4.avi'];
    else
