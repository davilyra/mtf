...
First version completed: 08/01/2009
...
Output: stress, pdms thickness, cell thickness
%}

function [FilmStress,pdms_thick,cell_thick_mod] = FilmStressCalc(RCurve,FilmLengthMet,num_films,frames,filename2)

global Cloc cell_thick bound thick
prompt = cell(2,1);
prompt{1} = 'PDMS layer thickness (micrometers)';
prompt{2} = 'Cell layer thickness (micrometers)';
%Ask the user for the thickness of the PDMS layer in micrometers &cell
%layer thickness
user_input_thicknesses = inputdlg(prompt,'Enter layer thicknesses');
pdms_thick = str2num(user_input_thicknesses{1});
%=input('Enter PDMS layer thickness in micrometers: ');
%Change the units to meters
pdms_thick=pdms_thick.*1e-6;

cell_thick_mod=str2num(user_input_thicknesses{2});
%cell_thick_mod=input('Enter cell layer thickness in micrometers: ');
cell_thick_mod=cell_thick_mod.*1e-6;
...
for filmcount=1:num_films
    ...
    %Material property of the cell (chosen)
    Ecell=30e3;
    Ccell=Ecell./6;
