...
if Choose == 0
    ...
    [file_cond,path_base] = uigetfile({'*.txt';'*.*'},'Please pick a text file with the condition names...','G:\Anya\');
    ...
    for cond_count = 1:NumCond
        ...
        [file_mX,pathX]=uigetfile({'*_StressDatafile_analyzed.mat';'*.*'},['Select the stress data files for ' condname_temp ' ...'],path_base,'MultiSelect','on');
        ...
        for filecount=1:cond_nf
            ...
            path_and_fileX=[pathX fileX] ;
            %load the file into a data variable
            data=load(path_and_fileX);
            cond(cond_count,cond_nf).pre_stress = data.pre_stress;
            cond(cond_count,cond_nf).abs_max_stress = data.abs_max_stress;
            cond(cond_count,cond_nf).mean_freq = data.mean_freq;
            cond(cond_count,cond_nf).num_films = data.num_films;
            ...
            pre_stress(cond_count,cond_n_temp:cond_n(cond_count)) = cond(cond_count,cond_nf).pre_stress;
            sys_stress(cond_count,cond_n_temp:cond_n(cond_count)) = cond(cond_count,cond_nf).abs_max_stress;
            cond_n_temp = cond_n(cond_count)+1; %update the temporary cond number
        end
    end
    ...
    base_diastolic_stress = mean(pre_stress(control,1:cond_n(control)));
...
%         end
    ...
    save(filename_user,'cond','freq','ave_freq','ave_cont_to_base_stress','condname_str','NumCond','cond_n','er_freq','er_cont_to_base_stress');
else
